...
#define M_PI_2 9.8696044010893586188	//PI^2
...
calculate_func(struct calculation_arguments const* arguments, struct options const* options, int i, int j)
{
	...
	{
		return ((2 * M_PI_2) * h * h * sin(M_PI * h * (double)i) * sin(M_PI * h * (double)j)) / 4;	//M_PI_2 verwenden damit nicht jedes mal PI quadriert werden muss
	}
	else
	{
		return 0.0;
...
calculate(struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options)
{
	...
	while (term_iteration > 0)
	{
		...
		//Reihenfolge switchen (siehe Vorlesung 2, 47)
		/* over all rows */
		for (i = 1; i < N; i++)
		{
			/* over all columns */
