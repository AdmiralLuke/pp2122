	...
	while (term_iteration > 0)
	{
		...
		if(options->termination == TERM_PREC){
			if(!first_iteration && options->rank < options-> size - 1){
				MPI_Recv(&term_iteration, 1, MPI_INT, MPI_ANY_SOURCE, 3, MPI_COMM_WORLD, NULL);
		

				if(options->rank < options->size - 2){
					MPI_Send(&term_iteration, 1, MPI_INT, options->rank + 1, 3, MPI_COMM_WORLD);
				}
				...
				if(options->rank > 0){
					MPI_Recv(&maxresiduum, 1, MPI_INT, options->rank - 1, 2, MPI_COMM_WORLD, NULL);
				}
			}
		}
		...
		if(options->rank > 0){
			MPI_Recv(Matrix[m2][0], N + 1, MPI_DOUBLE, options->rank - 1, 0, MPI_COMM_WORLD, NULL);
		}
		
		/* empfange Zeile vom nächsten Rang, wenn die erste Iteration abgeschlossen ist */
		if(!first_iteration && options->rank < options->size - 1){
			MPI_Recv(Matrix[m2][arguments->ranks - 1], N + 1, MPI_DOUBLE, options->rank + 1, 1, MPI_COMM_WORLD, NULL);
		}

		/* over all rows */
		for (i = 1; i < arguments->ranks - 1; i++)
		{
			double fpisin_i = 0.0;
			...
			if(i == 1 && options->rank > 0){
				MPI_Send(Matrix[m2][1], N + 1, MPI_DOUBLE, options->rank - 1, 1, MPI_COMM_WORLD);
			}
		}
		...
		if(options->rank < options->size - 1){
			MPI_Send(Matrix[arguments->ranks - 2], N + 1, MPI_DOUBLE, options->rank + 1, 1, MPI_COMM_WORLD);
		}
		...
		if (options->termination == TERM_PREC)
		{
			...
			if (maxresiduum < options->term_precision && options->rank == options->size - 1)
			{
				term_iteration = 0;
				MPI_Send(&term_iteration, 1, MPI_INT, 0 , 3, MPI_COMM_WORLD);
			} else{
				MPI_Send(&term_iteration, 1, MPI_INT, 0 , 3, MPI_COMM_WORLD);
			}

			/* Sende aktuellen maximalen Fehler and nächsten Rang */
			MPI_Send(&maxresiduum, 1, MPI_INT, options->rank + 1, 2, MPI_COMM_WORLD);
		}
		else if (options->termination == TERM_ITER)
		{
			term_iteration--;
